...
#define LONG_TOUCH_DELTA    500.0f
    ...
    if (rtFridge.containsPoint(v2Local))
    {
        ...
        for (int i = 0; i < FOOD_COUNT; i++)
        {
            ...
            if (pFood->getBoundingBox().containsPoint(v2Local))
            {
                ...
                scheduleOnce(schedule_selector(CFridge::longTouchFood), LONG_TOUCH_DELTA / 1000);
                return;
            }
        }
...
void CFridge::touchMove(Touch *touch)
{
    ...
    if (m_bFoodTouched)
    {
        ...
        if (lCurTime - m_lTouchBeginTime > LONG_TOUCH_DELTA)
        {
            if (!m_bMoved || m_bDraged)
            {
                // 拖起食材
                dragFood(touch->getLocation());
            }
        }
        else
        {
        ...
        //划出冰箱门
        Vec2 v2Local = this->convertTouchToNodeSpace(touch);
        Rect rtFridge = m_pIceBox->getBoundingBox();
        if (!rtFridge.containsPoint(v2Local))
        {
            m_pScrollView->setScrollEnabled(false);
...
void CFridge::touchEnd(Touch *touch)
{
    log("touchEnd");
    if (!m_bTouched) return;
    unschedule(schedule_selector(CFridge::longTouchFood));
    m_bTouched = false;
    m_bFoodTouched = false;
    m_pScrollView->setScrollEnabled(true);
...
void CFridge::dragFood(Vec2 v2Position)
{
    m_bDraged = true;
...
void CFridge::longTouchFood(float fEscapeTime)
{
    if (!m_bMoved && m_bFoodTouched)
    {
        // 拖起食材
        dragFood(m_pContentLayer->convertToWorldSpace(m_pCurTouchedFood->getPosition()));
    }
}
